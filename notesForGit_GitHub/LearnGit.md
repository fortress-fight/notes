...
1. 查看提交历史
    ...
    - git log
        ...
        - 参数：
            ...
            2. --stat
                如果需要查看简易信息，需要使用 `git log --stat` 命令，这样就不会显示差异细节,而是显示所有被修改过的文件、有多少文件被修改了以及被修改过的文件的哪些行被移除或是添加了。 在每次提交的最后还有一个总结。
            3. --graph
                `git log --graph` 将会以时间线的方式显示整个操作流程（分支，合并的历史）；
            4. --pretty
                这个选项可以指定使用不同于默认格式的方式查看提交历史，需要配合参数使用：（oneline, short, full, fuller）示例: `git --pretty=online`;
                oneline -- 一行显示，short -- 显示简化的信息，fuller -- 显示更完整的信息；
                对此还可以定制形式是，使用参数 format，示例：`git log --pretty=format:"%h - %an, %ar:%s"`
                图示：
                    ![format 参数](img/2017-09-06-09-16-43.png)
            5. 其他
                ![git log 参数](img/2017-09-06-09-18-30.png)
                
        - 注：多个参数可以并列使用

        - 限制输出长度
            在 git 命令中，存在多个参数来限制 git log 的输出长度，除了上述中的 `git log -2` 来输出指定个数的方式以外，还存在其他比较常用的参数，比如:

            1. 以时间为条件 `git log --since=2.week` (这个命令可以在多种格式下工作，比如说具体的某一天 "2008-01-15"，或者是相对地多久以前 "2 years 1 day 3 minutes ago"。)
            2. 以路径为条件 `git log <path>` 仅仅显示路径下的文件的修改
            3. 其他：
                ![限制输出长度](img/2017-09-06-09-29-38.png)
